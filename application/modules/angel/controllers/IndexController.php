        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if ($isEmailExist) {
                    ...
                    $result = $userModel->registerUser($email, $password, Zend_Session::getId(), false);
                }
            } catch (Angel_Exception_User $e) {
//                $this->_helper->json($e->getDetail());
            ...
            if ($result) {
                $this->_redirect($this->view->url(array(), 'login') . '?register=success');
            } else {
                $this->view->error = $error;
            }
        } else {
            // GET METHOD
            $this->view->title = "注册芝士电视";
        }
