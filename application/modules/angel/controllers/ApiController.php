...
class Angel_ApiController extends Angel_Controller_Action {
    ...
    public function loadMeetAction() {
        ...
        if ($result) {
            ...
            $data = array("meet_id"=>$id, "meet_text"=>$result->meet_text, "start_date"=>$start_date, "end_date"=>$end_date, "selected_date"=>$result->selected_date, "year"=>$result->year, "month"=>$result->month, "day"=>$result->day,"address"=>$result->address, "remark"=>$result->remark, "users"=>$users, "users_id"=>$users_id, "proposer_id"=>$result->proposer_id);
...
//        $this->_helper->json(array('data' => count($voteDates), 'code' => 0)); exit;
        $max_date = "";
        $max_num = 0;
        //冒泡获取最大投票数日期
